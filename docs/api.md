...
| POST   | `/api/nodes/{id}/upgrade-agent` | P10b:向在线节点下发 Agent 一键升级(见下) |
...
- `tunnel_id` 创建可选,把该规则挂到一条隧道作为业务入口规则;**仅创建时设定,PATCH 不可改**。P7 起普通用户可挂**被授权的**隧道(未授权 → 400「无权使用该隧道」);隧道的管理(增删改/重启)仍 admin-only。给定时 `node_id` 必须等于该隧道入口(ordinal 0)节点,否则 → 400(message 含「入口」)。挂隧道的入口规则其 `listen_port` 对 **admin 豁免节点池范围检查**(隧道入口是面向业务的端口);P7 起普通用户的隧道规则**仍受节点池约束**(防绕开端口管控)。保留端口红线对所有人生效(22/80/443/3306/5432 一律拒绝)。
- `max_connections`(P10a 新增,创建可选):规则级并发连接上限,**仅 TCP 生效**(UDP 无连接语义,tcp_udp 规则只限 TCP 侧)。PATCH 传 `0` = 清除(恢复不限);负数 → 400。**仅 admin 可设/可改**(普通用户传该字段一律 400)。Agent 端达到上限时新连接被直接断开(计入 `error_count` 并打 warn 日志);上限下调后存量连接保留至自然断开,短暂可能超员。隧道规则仅在 entry 节点计数;导入导出不携带该字段。响应含 `max_connections`(不限 → `null`)。
...
`POST /api/nodes/{id}/upgrade-agent`(admin):

- 向**在线**节点下发 `UpgradeAgent` 命令(目标版本 = 面板自身版本,amd64/arm64 两个
  sha256 由面板现读 `${PANEL_DATA_DIR}/agent-dist/node-agent-linux-<arch>` 计算)。
- Agent 收到后:版本一致 → 幂等跳过;否则按自身 arch 从
  `{PANEL_PUBLIC_BASE_URL}/dist/node-agent-linux-<arch>` 下载 → 校验 sha256 →
  旧二进制改名 `.bak` → 新二进制原子 rename 就位 → `exec` 重启(规则已落盘,重启自动
  恢复;exec 失败自动回滚 `.bak` 再 exec 旧版)。全程无 shell,仅 unix 支持。
- 前置:`PANEL_PUBLIC_BASE_URL` 已配置且 agent-dist 目录至少有一个 arch 的二进制,
  否则 400;节点不在线 → 400「节点不在线」(未遂也记 audit,success=false)。
- **存量节点注意**:P10b 之前安装的 systemd unit 带 `ProtectSystem=strict` 但
  `ReadWritePaths` 不含 `/usr/local/bin`,Agent 写盘会 EROFS 静默失败(仅 Agent 日志
  可见)。需手动在 unit 的 `ReadWritePaths` 追加 `/usr/local/bin` 并 `daemon-reload`
  后重启,新装节点的 unit 模板已包含。
- 升级 exec 重启会**切断该节点全部存量连接**(规则自动恢复,连接不会);旧二进制
  保留为同目录 `.bak`(有意行为,手动回滚保底)。下载有 120s 超时与 256MB 上限,
  下载/校验期间心跳照常(升级在后台任务执行,重复命令被 in-progress 守卫忽略)。
- 幂等:目标版本 = 自身版本,或自身二进制 sha256 已等于目标 sha → 跳过。
- 响应 `{ ok, dispatched, target_version }`;dispatch 成功 ≠ 升级成功(单向命令流
  无 ack),结果通过节点列表 `agent_version` 列观察(Agent 重启重注册后上报新版本)。
  记 audit `node.upgrade_agent`。
