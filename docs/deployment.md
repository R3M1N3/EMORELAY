...
| `PANEL_PUBLIC_HOST` | — | 内置 CA 写入 server 证书的对外主机名(Agent 校验);留空仅含 `127.0.0.1`/`localhost` |
...
1. **首次 P3a 启动前**先把 `PANEL_PUBLIC_HOST` 设为面板对 Agent 可达的主机名(如 `relay.example.com`)。内置 CA 据此写 server 证书 SAN;留空则只含 `127.0.0.1`/`localhost`,远程 Agent 握手会因 SAN 不匹配失败。
