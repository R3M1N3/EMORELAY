...
**当前阶段：尚未脚手架化（pre-scaffold）。** 仓库仅包含 `CLAUDE.md`（本文件）和 `plan.md`（蓝本），未初始化为 git 仓库，无 Cargo workspace、无 `web/`、无 `migrations/`。

下一步是按 plan.md 第十二节「开发顺序」从初始化 monorepo 开始构建。每完成一个原子单元后必须按上文「代码完成后必须使用子代理 review」流程审查。
...
项目尚未脚手架化，**目前没有任何 build / test / run 命令可用**。脚手架化完成后，预期的命令出口（按 plan.md 推断）应位于：

- Rust workspace：根 `Cargo.toml` → `cargo build` / `cargo test` / `cargo run -p panel-server` / `cargo run -p node-agent`
- 前端：`web/` 下 `npm run dev` / `npm run build`
- 数据库迁移：`sqlx migrate run`（基于 `migrations/`）
- 一键启动：根 `docker-compose.yml` → `docker compose up -d`（验收标准之一）

每完成一个阶段必须运行对应的构建/测试命令并自行修复错误，再进入下一阶段。

## 实施导航

不要试图一次性铺开所有代码。严格按 plan.md **第十二节「开发顺序」**的 20 步推进。每步完成后：
1. 跑对应阶段的 build / test 验证（前端 `npm run build`、Rust `cargo build && cargo test`、迁移 `sqlx migrate run`）。
2. spawn 子代理走 `superpowers:code-reviewer` 流程。
3. 通过后再进入下一步。
